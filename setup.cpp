 ...
 */
#define MASTER_CAL 0 //10_sp7etd
#define USB_CAL 8 //10_sp7etd
...
void setupFreq(){
  int knob = 0;
  //int32_t prev_calibration;//10_sp7etd
...
//  strcat(b, "Hz");
//  printLine2(b);     
    ltoa(calibration, b, 10);//10_sp7etd_10 - copied from few lines below to display previous before knob move
    displayText(b, 100, 140, 100, 26, DISPLAY_CYAN, DISPLAY_NAVY, DISPLAY_WHITE);//10_sp7etd_10 display previous before knob move
...
void setupBFO(){
  int knob = 0;
  //unsigned long prevCarrier;// 10_sp7etd
   
  //prevCarrier = usbCarrier;// 10_sp7etd

  displayDialog("Set BFO", "Press TUNE to Save"); 
  EEPROM.get(USB_CAL, usbCarrier); // 10_sp7etd
  // usbCarrier = 11053000l; // 10_sp7etd
